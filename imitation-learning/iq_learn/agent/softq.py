...
class SoftQ(object):
    def __init__(self, num_inputs, action_dim, batch_size, args):
        ...
        self.log_alpha = torch.tensor(np.log(agent_cfg.init_temp).astype(np.float32)).to(self.device)
    ...
    def choose_action(self, state, sample=False):
        if isinstance(state, (LazyFrames, GymLazyFrames)):
            state = np.array(state) / 255.0
        state = torch.FloatTensor(state.astype(np.float32)).to(self.device).unsqueeze(0)
        with torch.no_grad():
            q = self.q_net(state)
    ...
    def infer_q(self, state, action):
        if isinstance(state, (LazyFrames, GymLazyFrames)):
            state = np.array(state) / 255.0
        state = torch.FloatTensor(state.astype(np.float32)).to(self.device).unsqueeze(0)
        action = torch.FloatTensor(np.array([action], dtype=np.float32)).unsqueeze(0).to(self.device)
